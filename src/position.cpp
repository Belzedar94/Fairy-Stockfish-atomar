...
bool Position::legal(Move m) const {
  ...
  // Illegal move for royal immunity: Can't capture each other
  if (royal_immunity_blast() && (type_of(piece_on(from)) == COMMONER && (type_of(piece_on(to)) == COMMONER)))
      return false;
  ...
  if (var->extinctionPseudoRoyal)
  {
      ...
      if (capture(m) && blast_on_capture() && !royal_immunity_blast())
          occupied &= ~((attacks_bb<KING>(kto) & (pieces() ^ pieces(PAWN))) | kto);
      if (capture(m) && royal_immunity_blast())
          occupied &= ~((attacks_bb<KING>(kto) & (pieces() ^ pieces(PAWN) ^ pieces(COMMONER))) | kto);
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  if (captured && blast_on_capture())
  {
      ...
      if (royal_immunity_blast()) {
          blast = (attacks_bb<KING>(to) & (pieces() ^ pieces(PAWN) ^ pieces(COMMONER))) | to; 
          if (type_of(piece_on(to)) == COMMONER)
            blast = (attacks_bb<KING>(to) & (pieces() ^ pieces(PAWN) ^ pieces(COMMONER))); // Royal Immunity Clause
      }
      while (blast)
      {
          ...
          if (type_of(bpc) != PAWN || (type_of(bpc) != COMMONER && royal_immunity_blast()))
...
Value Position::blast_see(Move m) const {
  ...
  if (royal_immunity_blast()) {
      blast = ((attacks_bb<KING>(to) & ~pieces(PAWN) & ~pieces(COMMONER)) | fromto) & pieces(); 
      if (type_of(moved_piece(m))==COMMONER)
        blast = ((attacks_bb<KING>(to) & ~pieces(PAWN) & ~pieces(COMMONER))) & pieces(); // Royal Immunity Clause
  }
